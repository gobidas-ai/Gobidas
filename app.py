    ...
    with st.chat_message("assistant"):
        ...
        if any(word in prompt.lower() for word in trigger_words):
            with st.spinner("Nano Banana is rendering..."):
                ...
                # Fetching the image as bytes so it's permanent in history
                gen_url = f"https://pollinations.ai/p/{clean_prompt.strip().replace(' ', '_')}?width=1024&height=1024&nologo=true"
                response = requests.get(gen_url)
                
                if response.status_code == 200:
                    img_bytes = response.content
                    st.image(img_bytes, caption="Generated by Gobidas")
                    
                    # Add download button
                    st.download_button(label="Download Image", data=img_bytes, file_name="gobidas_gen.png", mime="image/png")
                    ...
                    st.error("Engine failed to respond. Try a different prompt.")
        else:
            try:
                if img_file:
