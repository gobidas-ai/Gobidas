...
<style>
    ...
    .overlay-content {{ max-width: 100%; max-height: 85%; object-fit: contain; }}
    .exit-btn-wrap {{ position: fixed; bottom: 30px; z-index: 1000000; width: 250px; }}
</style>
""", unsafe_allow_html=True)

# --- 2. OVERLAY LOGIC (CREATOR & VERY NICE MAN) ---
...
if st.session_state.creator_info_active:
    ...
    <div class="overlay-container">
        <img src="data:image/png;base64,{sec_img_base64}" class="overlay-content">
        <audio autoplay loop><source src="data:audio/mp3;base64,{sec_audio_base64}" type="audio/mp3"></audio>
    </div>
    """, unsafe_allow_html=True)
...
if st.session_state.nice_man_active:
    st.markdown(f"""
    <div class="overlay-container">
        <video class="overlay-content" autoplay loop>
...
if "user" not in st.session_state:
    ...
    with col:
        mode = st.radio("Access", ["Log In", "Sign Up"], horizontal=True, label_visibility="collapsed")
        u = st.text_input("Username", placeholder="Username")
        p = st.text_input("Password", type="password", placeholder="Password")
        agree = st.checkbox("I agree to the Global Terms of Service")
...
with st.sidebar:
    st.markdown(f"### Active: **{st.session_state.user}**")
    ...
    img_file = st.file_uploader("Visual Analysis", type=['png', 'jpg', 'jpeg'])
    st.divider()
